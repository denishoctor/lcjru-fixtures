...
- **Home page** — at-a-glance summary when no team is selected:
  ...
  - **Last weekend's results** — scored matches only, ordered by age grade youngest → oldest (kickoff time breaks ties). The weekend shown is the most recent one with *any* results — chosen independent of the Minis/Juniors filter (which only narrows the list within it), and walking back up to 6 weeks so a school-holiday or bye weekend doesn't render an empty section
  ...
  - **Group → team selector** — the toolbar leads with an **All · Minis · Juniors** pill control (progressive: at rest just the three pills; tapping a group reveals that group's team chips inline in the same row). The chosen group also narrows the *This weekend* and *Last weekend's results* lists (Minis = U6–U9, Juniors = U10–U15; sourced from `MINIS_SLUGS`). The choice persists across visits and is shareable via the URL hash (`#minis` / `#juniors`); tapping the active pill returns to rest. *Coming up* club events stay visible for every group. Since Minis aren't scored, selecting *Minis* hides the results section. Picking a team collapses the toolbar to the header team-pill.
